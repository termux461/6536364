...
## 🔄 Автопродление (визуальный режим)

В меню ключа есть тумблер **🔄 Автопродление: вкл/выкл**.

1. Включите тумблер **Автоплатёж (рекуррент)** в блоке YooKassa (**Настройки → Платёжные системы**) — он показывает кнопку автопродления в меню ключа.
2. В меню ключа (**Мои ключи → ключ**) пользователь одной кнопкой включает или выключает автопродление. Переключение мгновенное, **без обращения к ЮKassa/ЮMoney и без списаний** — это визуальный статус.
3. Реальная логика автосписания (продление с внутреннего баланса / рекуррент по карте) присутствует в коде (`charge_key_autopay`), но **не вызывается** — планировщик в этом режиме ничего не списывает. Чтобы включить реальные автосписания позже, нужно снять ранний `return` в `_maybe_process_autopayments` (файл `scheduler.py`).

> Обычные платежи ЮKassa этот режим не затрагивает: сохранение карты (`save_payment_method`) не запрашивается.
