...
`/keygen`, `/system/*`. Если операции нет в документированном API — поднимается
`RemnawaveUnsupportedOperation`, эндпоинты не выдумываются.

### Две версии панели: API v2 и v3

Поддерживаются оба мажора. **Пути одинаковые** — различается то, что внутри, и это собрано
в `app/services/remnawave/dialects.py`:

| | v2 | v3 |
|---|---|---|
| поле сертификата в `GET /keygen` | `pubKey` | `secretKey` |
| блок xHTTP в host | `xHttpExtraParams` | `xhttpExtraParams` |
| привязка host → node | нет такого поля | `nodes: [uuid]` |
| теги host | `tag` (строка) | `tags` (массив) |
| POST при создании | `200` | `201` |
| DELETE | `200` + тело | `204`, пусто |
| фоновые операции | `200` + счётчики | `202`, пусто |
| `GET /system/metadata` | нет | есть, отдаёт версию |

Конверт успеха `{"response": ...}` одинаков в обоих.

**Определение версии.** При подключении клиент делает один запрос `GET /system/metadata`.
Эндпоинт появился в v3, поэтому его отсутствие (`404`) — это положительный ответ «панель v2»,
а не сбой. `401/403` не считается ответом о версии: отклонённый токен пробрасывается наверх.
Если панель недоступна вообще — берётся дефолт (v3), и первый настоящий запрос сообщит
реальную причину.

Результат сохраняется в `remnawave_instances.api_version` (миграция `0005`), поэтому воркер
не переспрашивает на каждом шаге. Значение можно закрепить вручную — `auto` (по умолчанию),
`v2` или `v3`; закреплённая версия отключает проверку целиком. Это нужно, если панель стоит
за прокси, который съедает `404`.

```python
async with RemnawaveClient(url, token) as client:        # определит сам
    ...
async with RemnawaveClient(url, token, api_version="v2") as client:  # закрепить
    ...
```

**Update-запросы идут в коллекцию.** `PATCH /nodes/{uuid}` не существует ни в v2, ни в v3 —
uuid передаётся в теле (`UpdateNodeRequestDto.uuid`), а метод вызывается на `/nodes`. То же
для `/hosts`, `/config-profiles` и `/internal-squads`.

Источники: OpenAPI-документ панели v2.1.13 и контракты v3 из `remnawave/backend`
(`libs/contract`). Ничего не выведено из поведения «на глаз».
